
export const mainheaderlogos = [
    {
        avatar: require("./images/flights.png"),
        name: "Flight",
        path: "/"
    },
    {
        avatar: require("./images/hotel.png"),
        name: "Hotel",
        path: "/hotel"

    },
    {
        avatar: require("./images/hotelpackages.png"),
        name: "Holiday Packages",
        path: "/holidaypackages"
    },
    {
        avatar: require("./images/bus.png"),
        name: "Bus",
        path: "/bus"
    },
    {
        avatar: require("./images/homestays.png"),
        name: "Homestays",
        path: "/homestays"
    },
    {
        avatar: require("./images/trains.png"),
        name: "Trains",
        path: "/trains"
    },
    {
        avatar: require("./images/taxi.png"),
        name: "Taxi",
        path: "/taxi"
    },
    {
        avatar: require("./images/forex.png"),
        name: "Forex",
        path: "/forex"
    },
    {
        avatar: require("./images/charterflights.png"),
        name: "Charter Flights",
        path: "/charterflights"
    },
    {
        avatar: require("./images/activites.png"),
        name: "Activites",
        path: "/activites"
    },

]


export const cardimages = [
    {
        avatar:require("./images/card/bali.png"),
        name: "Bali"
    },
    {
        avatar:require("./images/card/thailand.png"),
        name: "Thailand"
    },
    {
        avatar:require("./images/card/nepal.png"),
        name: "Nepal"
    },
    {
        avatar:require("./images/card/maldives.png"),
        name: "Maldives"
    },
    {
        avatar:require("./images/card/paris.png"),
        name: "Paris"
    },
    {
        avatar:require("./images/card/switzerland.png"),
        name: "Switzerland"
    },
    {
        avatar:require("./images/card/moscow.png"),
        name: "Mascow"
    },

]